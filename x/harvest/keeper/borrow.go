...
// GetPendingBorrowBalance gets the user's total pending borrow balance (= outstanding borrow balance + pending interest)
func (k Keeper) GetPendingBorrowBalance(ctx sdk.Context, borrower sdk.AccAddress) sdk.Coins {
	borrow, found := k.GetBorrow(ctx, borrower)
	if found {
		totalNewInterest := sdk.Coins{}
		...
		borrow.Amount.Add(totalNewInterest...)
	}
	return sdk.Coins{}
}
