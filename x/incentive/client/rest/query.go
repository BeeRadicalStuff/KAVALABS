...
func registerQueryRoutes(cliCtx context.CLIContext, r *mux.Router) {
	...
	r.HandleFunc(fmt.Sprintf("/%s/nextaccount", types.ModuleName), queryNextAccountHandlerFn(cliCtx)).Methods("GET")
}
...
func queryNextAccountHandlerFn(cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		cliCtx, ok := rest.ParseQueryHeightOrReturnBadRequest(w, cliCtx, r)
		if !ok {
			return
		...
		route := fmt.Sprintf("custom/%s/%s", types.QuerierRoute, types.QueryGetNextAccount)
